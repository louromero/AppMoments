...
class _DetalleAlbumPageState extends State<DetalleAlbumPage> {
  ...
  void _cargarDatos() async {
    ...
    final f = await MomentoDatabase.instance.obtenerFotosAlbum(_albumActual.id!);
    setState(() {
      _miembros = m;
      _fotos = f;
    });
        ...
        child: Column(
          ...
          children: [
            ...
            TextButton(
              ...
              child: const Text("Cerrar", style: TextStyle(color: Colors.white, fontSize: 18)),
            )
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    final bool tienePermisos = userLogueado.esAdministrador == 1 || userLogueado.id == _albumActual.administradorId;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          if (tienePermisos) ...[
            IconButton(icon: const Icon(Icons.edit), onPressed: _editarInfoCompleta),
            IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: _confirmarBorrado),
          ]
        ],
      ),
  ...
  Widget _buildInfoCard() {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: Colors.orange.shade100, blurRadius: 10)]),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(_albumActual.titulo, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.orange)),
          const Divider(),
          Row(children: [
            const Icon(Icons.calendar_today, size: 18, color: Colors.grey),
            const SizedBox(width: 10),
            Text("Fecha: ${_albumActual.fecha.day}/${_albumActual.fecha.month}/${_albumActual.fecha.year}")
          ]),
  ...
  Widget _buildMapaSeccion() {
    return Container(
      height: 200,
      decoration: BoxDecoration(border: Border.all(color: Colors.orange.shade300, width: 2), borderRadius: BorderRadius.circular(15)),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(13),
        child: FlutterMap(
          options: MapOptions(initialCenter: LatLng(_albumActual.lat, _albumActual.lng), initialZoom: 15),
          children: [
            TileLayer(urlTemplate: "https://tile.openstreetmap.org/{z}/{x}/{y}.png", userAgentPackageName: 'com.momento.app.final'),
            MarkerLayer(markers: [
              Marker(point: LatLng(_albumActual.lat, _albumActual.lng), width: 40, height: 40, child: const Icon(Icons.location_pin, color: Colors.red, size: 40))
            ]),
          ],
        ),
      ),
    ...
    return Column(
      children: [
        ...
        if (!tienePermisos) ...[
          ...
          TextButton(onPressed: () => _expulsar(myId), child: const Text("SALIR DEL ÁLBUM", style: TextStyle(color: Colors.red))),
        ]
      ],
    );
  }
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 15),
                ListTile(
                  title: Text("Fecha: ${fechaEditada.day}/${fechaEditada.month}/${fechaEditada.year}"),
                  trailing: const Icon(Icons.calendar_month),
                  onTap: () async {
                    final picked = await showDatePicker(context: context, initialDate: fechaEditada, firstDate: DateTime(2000), lastDate: DateTime(2100));
                    if (picked != null) setDialogState(() => fechaEditada = picked);
                  },
                ),
              ],
            ),
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            const Text("Miembros", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            Flexible(
              child: ListView.builder(
                ...
                itemBuilder: (context, i) {
                  ...
                  return ListTile(
                    ...
                    subtitle: Text(m.id == _albumActual.administradorId ? "Admin" : "Miembro"),
                  );
                },
              ),
            ),
